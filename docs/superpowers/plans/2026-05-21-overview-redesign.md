...
**Goal:** Replace the unused forum-mentions Overview page with an ops-focused dashboard showing global KPIs, an 8-tab summary grid, and a recent-syncs table — all driven by the `entries` table.

**Architecture:** Single file rewrite of `src/pages/Overview.tsx`. Fetches `fetchTabKpis` for all 8 `OPERATIONAL_TABS` in parallel plus `fetchSyncRuns(5)`. Aggregates globals client-side. No new queries, no new components, no real-time subscription.

**Tech Stack:** React 19, TypeScript strict, Tailwind v4, React Router v7, lucide-react, supabase-js

---

### Task 1: Rewrite `src/pages/Overview.tsx`
...
Run: `npm run build` from `Brands Partner Forum/`

Expected: no TypeScript errors. If there are errors related to unused imports from the old page (e.g. `MentionsTable`, `TimeSeriesChart`, `TopList`), they should not appear since those imports are no longer in the file. Any remaining TS error means a type mismatch — fix before proceeding.

- [ ] **Step 3: Commit**

```bash
git add src/pages/Overview.tsx
git commit -m "feat(overview): replace mentions KPIs with brand ops summary

- Global KPIs: total accounts, live reviews, removed, last sync
- 8-tab summary grid with live/removed bar per tab
- Recent syncs table (last 5 runs)
- Removes unused mentions-based charts and queries from Overview"
```
