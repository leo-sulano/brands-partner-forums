...
2. Check that platform, enter a reason, pick "Until a date" a week+ out, Save. Confirm: within ~1–2s the Schedule Status column shows `⛔ Paused` and its tooltip reads **"Manually paused" / "Reason: …" / "Resumes …"**. (Day-cell chip dimming for a manual pause only lands from the next weekly regeneration — on an already-generated current week the chips stay active-looking; that's expected, see the spec's Decision 1 correction note.)
...
Prepend a `CLAUDE.md` "Recent Changes" entry dated `2026-09-04` summarizing: manual `brand_platform_override` pauses are visible on the Schedule Planner grid again (reversing part of Tasks 320/321) and editable from the Schedule Status column via a **platform-scoped** `PlatformPauseModal`; shared `src/lib/platformPauseActions.ts` is the single write path for both surfaces; tooltip forks Auto-paused vs Manually paused; `PauseDaysModal` gained a durable-pause escalation button; frontend-only deploy (`git push origin main`), no migration / no edge-function redeploy. The entry must also record: (a) the manual-pause grid-vs-PMS window on an already-generated week (day chips stay active-looking until the next Monday regen even though the status-column pill/tooltip and the PMS Project Paused move are immediate); (b) that the `reloadSeq` refresh does a full `fetchRawEntriesByTab` per save (heavier than a bespoke `refreshPauseState`, deliberate — reuses the trusted realtime-INSERT refetch path); (c) that the Schedule Planner's `PlatformPauseModal` is platform-scoped (one platform per open, per the final-review fix wave) whereas Edit Brand Tab's stays brand-scoped.
