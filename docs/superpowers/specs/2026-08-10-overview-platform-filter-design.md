...
- Read from URL `?platform=` param (validated against `['tp','ag','cg','wo']`, anything else falls back to `'all'`), mirrored to `localStorage` under a dedicated key (e.g. `overview-platform-filter`), matching the read-on-mount/write-on-change pattern `ScoreSummaryPanel.tsx` already uses (lines 103-191 there) — omit the param from the URL when `'all'` to keep URLs clean, same as that page does for its default.
