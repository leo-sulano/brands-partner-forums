...
- **Star-rating histogram** (`computeScoreSummary`'s `counts: Record<Star, number>`/`unrated`, `scoreKeys`/`maxScore` lookups, `scoreSummary.ts:258-259,316-321`): confirmed with you directly — when 2+ selected platforms don't share the same `PLATFORM_MAX_SCORE`, the histogram is not computed/shown at all (no merging across incompatible scales). When every selected platform *does* share a max score, or exactly one is selected, the histogram computes normally. `ScoreSummaryPanel.tsx`'s render (around `result.excludedRows`/`GroupedSummary`, `ScoreSummaryPanel.tsx:278-292`) gains a check for this and hides the histogram section (keeping Live/Removed/Success-Rate visible) with a short inline note, rather than a hard error, when the selected platforms' scales don't match.
