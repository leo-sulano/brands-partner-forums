...
## Task 325: Custom Platforms — let any user define a 5th review platform, plus its final-review fix wave
...
2 new tests (`queries.test.ts`'s reserved-name rejection for "Trust Pilot",
`customPlatformRegistry.test.ts`'s idempotency lock); full suite **1013** passing (1011 + 2);
`npm run build` clean.
...
the list. One incidental discovery, out of this fix wave's scope and not touched: `EditEntryModal`
does not surface a newly-created custom platform's fields (only `AddReviewAccountModal` does, per
the original plan's actual scope — the plan's own Task 7 verification note about `EditEntryModal`
needing "zero changes" was about it not *breaking*, not about it rendering brand-new columns
with no existing entry data yet; worth a follow-up look if custom platforms see real use). Cleanup
