...
export default function Sidebar({ open = false, onClose, collapsed = false, onToggleCollapsed }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        <aside
          className={`flex flex-col h-screen bg-[#1E2A6B] text-slate-100 transition-[width] duration-200 ease-in-out overflow-hidden ${collapsed ? 'w-16' : 'w-60'}`}
        >
          {header(collapsed)}
        ...
        {collapsed && (
          <aside
            inert={!hoverExpanded}
            className={`fixed inset-y-0 left-0 z-[45] w-60 flex flex-col bg-[#1E2A6B] text-slate-100 shadow-xl transition-opacity duration-200 ease-in-out ${
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-[45] flex">
          ...
          />
          {/* Panel */}
          <aside className="relative z-50 flex flex-col w-72 bg-[#1E2A6B] text-slate-100 h-full shadow-xl">
