...
export default function TabIcon({ tab, className }: Props) {
  ...
  if (resolved.kind === 'image') {
    return <img src={resolved.url} alt="" className={`rounded-[3px] border border-[#ffffff80] ${className ?? ''}`} onError={hideOnError} />;
  }
  ...
  if (resolved.kind === 'favicon') {
    ...
    return <img src={faviconUrl(resolved.domain)} alt="" className={`rounded-[3px] border border-[#ffffff80] ${className ?? ''}`} onError={hideOnError} />;
  }
