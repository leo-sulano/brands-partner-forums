...
import { leastLoadedDay, weeklyCompletion, completedBrandPlatformKey, PLATFORM_BADGE, PLATFORM_FULL_LABEL, unscheduledPlatforms, buildDateStatusIndex, hasDateEvidence, resolveDateEvidenceKind, resolvePmsSyncStatus, buildAgentIndex, buildLastPostIndex, trailingManualPauseDays, effectivePauseDays, hasNoScheduleThisWeek, pausableWeekdays, buildAgentAssignmentMap, resolveAgentForPlatform, resolveAgentForBrand, buildResolvedAgentIndex, weekdayColumnsInRange, columnsForWeek, currentWeekColumns, countActivePlatformSlots, type DateStatusIndex, type EntryDetails } from './scheduleUtils';
...
describe('effectivePauseDays', () => {
  ...
  it('returns explicitly paused days when there is no system-level pause', () => {
    expect(effectivePauseDays(row({ monday: 'paused', wednesday: 'paused', friday: 'active' }), false))
      .toEqual(['monday', 'wednesday']);
  });

  it('returns every day when a system-level pause is active and no day has its own status', () => {
    expect(effectivePauseDays(row({}), true))
      .toEqual(['monday', 'tuesday', 'wednesday', 'thursday', 'friday']);
  });

  it('lets an explicit active status override a system-level pause for that day', () => {
    expect(effectivePauseDays(row({ monday: 'active' }), true))
      .toEqual(['tuesday', 'wednesday', 'thursday', 'friday']);
  });

  it('does not require a run — a lone paused day still counts, unlike trailingManualPauseDays', () => {
    expect(effectivePauseDays(row({ friday: 'paused' }), false)).toEqual(['friday']);
  });

  it('returns empty for an undefined row with no system-level pause', () => {
    expect(effectivePauseDays(undefined, false)).toEqual([]);
  });
...
describe('buildDateStatusIndex', () => {
  ...
  it('does not add a details entry for a status that matches none of the four sets', () => {
    const entries = [entry({ Brands: 'WinMega', 'TP Review Status': 'On Pause', 'Trust Pilot': '2026-07-28', Account: 'someone' })];
    const { details } = buildDateStatusIndex(entries);
    expect(details.size).toBe(0);
  });

  it('does not add a details entry when the date is unparseable, matching the removed-set skip', () => {
    const entries = [entry({ Brands: 'WinMega', 'TP Review Status': 'Removed', 'Trust Pilot': null, Account: 'someone' })];
    const { details } = buildDateStatusIndex(entries);
    expect(details.size).toBe(0);
  });
});
...
describe('hasDateEvidence', () => {
  ...
  it('returns false against a completely empty index', () => {
    const empty: DateStatusIndex = { removed: new Set(), confirmed: new Set(), pending: new Set(), done: new Set(), details: new Map() };
...
describe('resolvePmsSyncStatus', () => {
  const emptyIndex = { removed: new Set<string>(), confirmed: new Set<string>(), pending: new Set<string>(), done: new Set<string>(), details: new Map<string, EntryDetails>() };
...
describe('buildLastPostIndex', () => {
  const entry = (data: Record<string, string | null>, updatedAt: string): Entry => ({
    id: 'x', tab: 'BITP', sheet_row_id: '1', data, updated_at: updatedAt, last_edited_by: 'dashboard', last_sync_tag: null,
  });

  it('maps a brand+platform to its one dated post', () => {
    const entries = [entry({ Brands: 'WinMega', 'TP Review Status': 'Published', 'Trust Pilot': '2026-08-01' }, '2026-08-01T00:00:00Z')];
    const index = buildLastPostIndex(entries);
    expect(index.get('winmega')?.tp).toEqual({ status: 'Published', dateISO: '2026-08-01' });
  });

  it('picks the latest-by-post-date entry when a brand+platform has multiple dated posts', () => {
    const entries = [
    ...
    const index = buildLastPostIndex(entries);
    expect(index.get('spinjo')?.tp).toEqual({ status: 'Published', dateISO: '2026-08-10' });
  });
  ...
  it('keeps each platform independent for a multi-platform brand', () => {
    ...
    const index = buildLastPostIndex(entries);
    expect(index.get('trybet.com')?.tp).toEqual({ status: 'Published', dateISO: '2026-08-01' });
    expect(index.get('trybet.com')?.ag).toEqual({ status: 'Removed', dateISO: '2026-07-20' });
  });
...
describe('countActivePlatformSlots', () => {
  ...
  const emptyIndex: DateStatusIndex = { removed: new Set(), confirmed: new Set(), pending: new Set(), done: new Set(), details: new Map() };
  ...
  it('for a past day, counts a brand+platform+day with evidence even when the plan has no row for it at all', () => {
    const index: DateStatusIndex = { removed: new Set(), confirmed: new Set(['a::tp::2026-08-17']), pending: new Set(), done: new Set(), details: new Map() };
    const cols = columnsForWeek(new Date('2026-08-17T00:00:00'));
