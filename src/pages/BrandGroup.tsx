...
// wo's set matches WO_COLUMNS in dynamicTabRegistry.ts exactly — the only tab
// shape that reaches this multi-platform code path with 'wo' present is a
// dynamic Brand Tab, whose columns are always generated by buildDynamicTabColumns.
const PLATFORM_OWN_COLS: Record<'tp' | 'ag' | 'cg' | 'wo', Set<string>> = {
  ...
  wo: new Set(['Wizard of Odds', 'WoO Review Status', 'Wizard of OddsScore added', 'WO Review Link']),
};
...
const PLATFORM_CARDS = [
  ...
  { key: 'wo' as const, label: 'Wizard of Odds', dot: 'bg-emerald-500' },
];
...
export default function BrandGroup() {
  ...
  const displayKpis = (() => {
    ...
    return { tp: countPlatform('tp'), ag: countPlatform('ag'), cg: countPlatform('cg'), wo: countPlatform('wo') };
  })();
  ...
  return (
    <div className="space-y-6 -mt-6 md:-mt-8 pt-[10px]">
      ...
      {activePlatforms.length > 1 && (() => {
        ...
        const cols = visibleCards.length === 1 ? 'sm:grid-cols-1' : visibleCards.length === 2 ? 'sm:grid-cols-2' : visibleCards.length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-4';
        return (
