...
import { deriveTabBrands, getTabPlatforms } from '../../../src/lib/tab-configs.ts';
import { fetchRawEntriesByTab, fetchTabHeaders, fetchRemovedPlatformBrands, fetchBrandPlatformOverrides, fetchScheduleHiddenBrands, fetchScheduleRestrictedBrands, fetchBrandAgentAssignments, fetchPublicHolidays, fetchBrandCatalog, invalidateTabCache } from '../../../src/lib/queries.ts';
...
import { buildAgentIndex, buildAgentAssignmentMap, buildNewBrandAddedAtMap, resolveAgentForPlatform } from '../../../src/lib/scheduler/scheduleUtils.ts';
...
// full coverage in schedulerService.test.ts. Brand derivation itself now
// calls the same deriveTabBrands (tab-configs.ts) TabScheduleSection.tsx and
// SchedulePlanner.tsx's landing-grid preview use, replacing what used to be
// its own hand-duplicated copy — folded in while adding brand_catalog
// support below so this cron path can't drift from the frontend on either.
export async function buildTabContext(tab: string, client: SupabaseClient): Promise<TabContext> {
  const [rawEntries, headers, removedPlatformBrandRows, overrideRows, hiddenBrandRows, restrictedBrandRows, holidayRows, catalogRows] = await Promise.all([
    ...
    // Same fail-open shape as the frontend's own catalog fetch — a
    // transient failure here just means a catalog-only brand doesn't get
    // (ramped and) generated by this particular cron run, not a broken run.
